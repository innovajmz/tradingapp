...
function dayTotals(dayTrades) {
  const gross = dayTrades.reduce((s, t) => s + Number(t.pnl), 0);
  const commission = dayTrades.reduce((s, t) => s + Number(t.commission || 0), 0);
...
export function computeMetrics(account, trades) {
  ...
  const totalPnl = days.reduce((s, d) => s + d.net, 0);
  const totalGrossPnl = days.reduce((s, d) => s + d.gross, 0);
  ...
  const todayCommission = todayDay ? todayDay.commission : 0;
  const dailyLossPct =
  ...
  const worstDay = days.reduce(
    (worst, d) => (d.net < worst.pnl ? { date: d.date, pnl: d.net } : worst),
    { date: null, pnl: 0 }
  );
